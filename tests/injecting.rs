...
//    }
    ...
    mod injects_nested_multi_file_mods_content {
        ...
        #[test]
        fn when_not_mocked_then_runs_normally() {
            assert_eq!("not mocked", multi_file_module::function(2));
        }

        #[test]
        fn when_mocked_then_runs_mock() {
            multi_file_module::function.mock_safe(|x| MockResult::Return(format!("mocked {}", x)));

            assert_eq!("mocked 2", multi_file_module::function(2));
        }
    }
}
